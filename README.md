...
`npm run package` produces installers in `desktop/release/`, and every release attaches them alongside a
`SHA256SUMS.txt` you can verify against. What each platform does on first run differs, so here is what
you will see and what to do about it.

**Windows.** The installer is **code-signed** via [SignPath](https://signpath.io) (free for open-source
projects), so it carries a genuine Authenticode signature. SmartScreen is reputation-based, not
detection: until a freshly-issued signature has been downloaded enough times, Windows may still show
*"Windows protected your PC — Microsoft Defender SmartScreen prevented an unrecognised app from
starting,"* with the **Run anyway** button hidden behind **More info**. This fades as the signed
installer earns download reputation; it is not a sign that anything is wrong with the file.

**macOS.** The `.dmg` is **not code-signed** with an Apple identity — it is **unsigned and unnotarised**
(the build is ad-hoc signed only, which is what lets Apple silicon launch it at all). Gatekeeper
therefore refuses the first launch: *"VoidCode cannot be opened because Apple cannot check it for
malicious software."* On macOS 15 and later the old Control-click → **Open** shortcut no longer clears
this — the documented path is **System Settings → Privacy & Security**, where an **Open Anyway** button
appears after the first refused launch. This is Apple's documented behaviour rather than something we
have observed, because nobody here has a Mac to test it on. Notarisation needs a paid Apple Developer
account and is a planned follow-up; `hardenedRuntime` is on in `electron-builder.yml` while notarisation
is not, which is the truthful configuration rather than a claim to something we do not have.
...
**Verifying what you downloaded.** Every release ships a `SHA256SUMS.txt` covering all its files. Check
your download against it before installing — `sha256sum --check --ignore-missing SHA256SUMS.txt` on
Linux and macOS, or `Get-FileHash <file> -Algorithm SHA256` on Windows — and on Windows you can also
confirm the signature with `Get-AuthenticodeSignature <file>` (it reports `Valid` for a signed build).
