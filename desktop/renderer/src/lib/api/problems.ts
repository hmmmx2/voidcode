 ...
 * Problems API Client
 *
 * Fetches problem data from `/v1/problems` through the transport seam in `client.ts`, which answers
 * it over IPC from `src/main/content/` — the curriculum is code in this repository, not rows in a
 * database (D0).
 *
 * Both sentences here were wrong rather than merely stale. It named the FastAPI backend, which no
 * longer serves this; and "replaces hardcoded mock data with database-driven content" has the
 * direction backwards now, since the move onto the desktop replaced database rows with source files.
 */

import type { Problem, TestCase } from "@/lib/mock-data";
// Was a second, local `API_BASE` that also sent no headers — so these two calls, the ones
// the workspace depends on most, went straight past the transport seam.
...
export async function fetchProblem(slug: string): Promise<ProblemDetail> {
  ...
  const problem: Problem = {
    ...
    ...(data.shapes !== undefined && data.shapes !== null
      ? { shapes: data.shapes as Problem["shapes"] }
      : {}),
    ...(data.plot !== undefined && data.plot !== null
      ? { plot: data.plot as Problem["plot"] }
      : {}),
    ...(typeof data.math === "string" ? { math: data.math } : {}),
  };
