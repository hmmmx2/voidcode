    ...
    <section>
      ...
      <Surface radius="panel" className="divide-y divide-line overflow-hidden">
        {visible.map((problem) => (
          <Link
            key={problem.id}
            // By slug, not `orderIndex`: a problem inserted mid-list renumbers every
            // position after it, and a stale numeric link opens the wrong problem successfully.
          ...
          >
            ...
            {/* The screen-reader label carries the progress too. A tick and a number is the
                whole state for a sighted user; "Not solved" alone would be strictly less
                information for everyone else. */}
            <span className="sr-only">
              {problem.isSolved
                ? "Solved"
                : problem.best !== undefined
                  ? `Not solved, best ${problem.best.passed} of ${problem.best.total} cases`
                  : "Not solved"}
            </span>
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 flex flex-wrap items-center gap-1.5">
                ...
                {/* What the store already knew and the row used to throw away. Someone at 3
                    of 4 cases is one edge case from done; someone at 0 of 4 is somewhere
                    else entirely, and both used to render as an identical empty circle. */}
                {!problem.isSolved && problem.best !== undefined && (
                  <>
                    <span aria-hidden className="text-[10px] text-ink-3">
                      ·
                    </span>
                    <span className="font-mono text-[10px] tabular-nums text-ink-3">
                      {problem.best.passed}/{problem.best.total} cases
                    </span>
                  </>
                )}
                {!problem.isSolved && problem.attemptCount > 0 && (
                  <>
                    <span aria-hidden className="text-[10px] text-ink-3">
                      ·
                    </span>
              ...
              {/* The same ratio as a rule, so the list scans without being read. Only for
                  problems with real progress — a zero-width bar on every untouched row would
                  be noise, and a full-width one on solved rows repeats the tick. */}
              {!problem.isSolved &&
                problem.best !== undefined &&
                problem.best.total > 0 &&
                problem.best.passed > 0 && (
                  <span
                    aria-hidden
                    className="mt-2 block h-px w-full max-w-[180px] bg-line"
                  >
                    <span
                      className="block h-px bg-ink-3"
                      style={{
                        width: `${(problem.best.passed / problem.best.total) * 100}%`,
                      }}
                    />
                  </span>
                )}
