...
export const NAV_ITEMS: { href: string; label: string; exact?: boolean }[] = [
  ...
  // The IDE, as a peer destination rather than a separate product. It was briefly its own
  // window because filesystem access was scoped to a window; that scoping moved onto the
  // folder-picker consent and the per-conversation tool set, so there is no longer a reason
  // for the user to manage two windows for one app.
  { href: "/build", label: "Code" },
];
...
export default function TopNavigation({
  ...
  // The web build fell back to the NextAuth session when the profile had no name.
  // There is no session here, so the profile is the only source.
  ...
  return (
    ...
    >
      ...
      {variant === "workspace" ? (
        /* Problem navigator — "Question: X/Y  <  >" */
            ...
            <Menu width="w-52">
              ...
              <div className="flex items-center gap-3 border-b border-line px-4 py-3.5">
                ...
                <div className="min-w-0">
                  ...
                  {userProfile?.email && (
                    <p className="truncate text-[11px] text-ink-3">{userProfile.email}</p>
                  )}
                </div>
              </div>
              ...
              {/* NO SIGN OUT. It was here with `onClick: () => undefined` — a
                  control that hovered, focused and did nothing when pressed.
                  This build has no auth for it to act on: `useUserId` returns a
                  fixed local id, and `auth.ts`, `middleware.ts`, `/api/auth/*`
                  and the login forms are all absent. A dead control is worse
                  than a missing one, because it makes the user doubt the click
                  rather than the app. The same button in `ProfileSidebar` went
                  at the same time. */}
            </Menu>
          )}
        </div>
