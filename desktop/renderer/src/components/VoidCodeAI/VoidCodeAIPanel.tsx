...
import { TUTOR_UNREACHABLE, NO_MODEL_INSTALLED } from "@/lib/copy";
import { useRuntimeStatus } from "@/lib/shell/useRuntimeStatus";
...
export default function VoidCodeAIPanel({
  ...
  /**
   * Images to send with the next turn, and only the next turn.
   *
   * Not kept in `messages`: the transcript is persisted, and storing base64 there would put
   * megabytes of image into the chat database for every diagram anyone ever pasted. The picture
   * is context for one question, the way the open file is.
   */
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  /**
   * The same list, readable synchronously.
   *
   * `attach` awaits a `FileReader` per file and checks the running count between them, so it
   * cannot read `attachments` — React has not committed the previous iteration's state yet, and
   * pasting four images at once would let every one of them see a count of zero. This is the
   * same bug the Build panel's drop handler hit twice, from the same cause.
   */
  const attachmentsRef = useRef<Attachment[]>([]);
  const [attachError, setAttachError] = useState<string | null>(null);
  /**
   * Said once, when the model that will answer cannot see.
   *
   * Not an error — the turn still goes — so it does not share `attachError`, which is styled as a
   * refusal and cleared on the next attach. This one persists while an attachment is held, because
   * it stays true for as long as that model is the installed one.
   *
   * Looked up on first attach rather than on mount: it costs an IPC round trip, and most turns
   * never attach anything.
   */
  const [visionNotice, setVisionNotice] = useState<string | null>(null);

  /**
   * Whether anything can answer, for the banner in the chat area.
   *
   * The same hook the status bar reads, rather than a bespoke `providers.list()` here — one probe
   * shape, one set of three states, and the two surfaces cannot end up disagreeing about whether a
   * model exists. It fetches once on mount and is not polled, which its own header explains.
   */
  const runtime = useRuntimeStatus();

  /**
   * Find out whether the model that will answer can see, and say so if it cannot.
   *
   * Found by driving the real app: attaching a diagram while Qwen3 8B was installed produced
   * "I currently cannot view or access attached images." The whole path worked and the learner
   * still got nothing, which is worse than a refusal because it looks like the feature failed.
   *
   * Failures here are swallowed. This is a courtesy on top of a turn that is about to be sent
   * anyway; a provider list that will not load is the send path's problem to report, and
   * reporting it twice from two places would be noise.
   */
  const noteVisionSupport = useCallback(async () => {
    try {
      const [active, catalogue] = await Promise.all([
        usableProvider(),
        window.host?.models.list() ?? Promise.resolve({ models: [] }),
      ]);
      setVisionNotice(visionNoticeFor(active?.model, catalogue.models));
    } catch {
  ...
  /**
   * Take images from a paste, a drop or the picker.
   *
   * Rejections are reported one at a time rather than collected: the first thing wrong with a
   * batch is what someone needs to fix, and a list of four complaints about the same oversized
   * screenshot reads as four problems.
   */
  const attach = useCallback(async (files: readonly File[]) => {
    setAttachError(null);
    void noteVisionSupport();
    for (const file of files) {
      const rejection = rejectionFor(file, attachmentsRef.current.length);
      if (rejection !== null) {
  ...
  /**
   * Where this request sits in the hosted backend's GPU queue, or null when it is not waiting.
   *
   * Only ever set from a `type: "queue"` frame, and cleared as soon as any answer content
   * arrives -- the position stops being true at that point, and a stale "3rd in line" sitting
   * above a streaming answer reads as a fault rather than as history.
   *
   * A local provider never produces one of these: there is one process on one machine serving
   * one person, so nothing to queue behind.
   */
  ...
  /**
   * The history, and the search behind it.
   *
   * `listSessions(20, 0)` was the whole of this, with no pagination and no search — so the
   * twenty-first conversation was unreachable from the UI, and the only way to find an old one
   * was to remember roughly when you had it. Searching in main covers every stored message
   * rather than filtering a page that had already dropped what you were looking for.
   *
   * A blank query lists everything up to the limit, so this one function serves both the
   * initial open and every keystroke after it.
   */
  const fetchSessions = useCallback(
    async (query = "") => {
      setIsLoadingSessions(true);
      try {
        const data = await searchSessions(query, 50, userId, "tutor");
  ...
  const handleDeleteSession = useCallback(
    async (sessionId: string) => {
      /**
       * Stop the stream *before* the row goes, when the row being deleted is the open one.
       *
       * `handleNewChat` already does this and this did not, which was survivable only while
       * the delete itself was broken — the request fell through to a 501 and nothing was ever
       * removed. With the route wired up the ordering becomes load-bearing: an in-flight SSE
       * loop keeps flushing tokens into a transcript that has been cleared, and the
       * post-stream `persistMessage` captured `sessionId` in a local, so it retries three
       * times over 1.5s appending to a session that no longer exists. Every one of those
       * fails on the foreign key and is swallowed into `console.error`.
       */
      // A previous failure must not stay on screen next to a delete that then succeeds.
      setDeleteError(null);
      ...
      try {
         ...
         *
         * The whole reason this button was dead for so long is that its only failure signal
         * went to a console nobody opens. A delete that fails now says so where the person
         * who pressed it is looking.
         */
        console.error("Failed to delete session:", err);
        setDeleteError(err instanceof Error ? err.message : "Could not delete that chat.");
      }
    },
  ...
  const handleSend = useCallback(async () => {
    // An image on its own is a question — "what is wrong with this loss curve" is often the
    // whole of it. Requiring text would make the attach button useless without a caption.
    if (!canSend(input, attachmentsRef.current.length) || isLoading) return;

    const userContent = input.trim() || "Please look at the attached image.";
    ...
    if (!sessionId) {
      try {
        ...
        /**
         * A title, taken from the question being asked.
         *
         * Nothing wrote one before, so every row in the history read "Untitled Chat" — a list
         * where each entry differs only by a relative timestamp is not a history, it is a
         * count. Build started doing this when its sessions landed; this is the same rule.
         *
         * The question rather than the problem name: two conversations about softmax are
         * distinguished by what was asked, not by which exercise they happened under.
         */
        const session = await createSession({
          problemId: isUuid ? pid : undefined,
          title: userContent.trim().slice(0, 80) || undefined,
          // Whose conversation this is. Without it the Build assistant's history and the
          // tutor's were one list, and neither was what anyone had asked for.
          surface: "tutor",
        }, userId);
    ...
    /**
     * Images ride on the newest turn only, and history stays text.
     *
     * A picture is context for the question it came with, like the open file — replaying it on
     * every subsequent turn would grow the request without adding anything, and the transcript
    ...
    // images to the next question, which is how someone ends up sending a diagram three times.
    ...
    try {
      const response = await fetch(`${API_BASE}/v1/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          /**
           * What the learner is on, so main can scope reference retrieval through the concept
           * graph. Declared on the channel in D3 and sent by nothing until now, which meant the
           * grounding worked but always searched unscoped.
           *
           * The problem id, which for a curriculum problem is its slug — the same string the
           * taxonomy maps. An id it does not recognise yields no concepts and costs relevance
           * rather than correctness, so this needs no guard beyond being present.
           */
          ...(context?.problem.id !== undefined ? { itemId: context.problem.id } : {}),
          ...
          for (const event of events) {
            ...
            if (parsed.type === "thinking") {
              ...
              thinkingMeta = {
                ...
                // Matches DISPLAY_THINKING_BUDGET in chat.ts, which is a denominator for the
                // progress bar rather than anything a provider was told — see the note there.
            ...
            // ── Token usage event ────────────────────────────────────
            // ── Queue position ───────────────────────────────────────
            // The hosted backend is busy and this request is waiting for a GPU slot. The frame is
            // a valid OpenAI chunk with an empty delta, so without this branch it would fall
            // through harmlessly below and render nothing at all.
            ...
            if (delta) {
              // The wait is over the instant the first token lands. Clearing here rather than at
              // stream end means the queue line never overlaps a visible answer.
      ...
      const errorMessage: ChatMessageType = {
        ...
        content: partial
          ...
          : `**Error:** ${err instanceof Error ? err.message : TUTOR_UNREACHABLE}`,
      };
      setMessages((prev) => {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-line bg-ide-panel">
      ...
      <div className="h-9 flex-shrink-0 flex items-center justify-between px-3 bg-ide-bar border-b border-line relative">
        ...
        {isHistoryOpen && (
          <ChatHistoryDropdown
            ...
            onSearch={(query) => void fetchSessions(query)}
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-6">
        {/**
         * No model, said before the user types rather than after.
         *
         * `TUTOR_UNREACHABLE` already covers the failure, but only once a send has failed — so the
         * sequence for a fresh install was: open the tutor, read an invitation to ask a question,
         * write one, wait, and then be told there is no model. The fact was available the whole time
         * (`useRuntimeStatus` is what the status bar reads) and simply never surfaced here.
         *
         * A BANNER RATHER THAN AN `EmptyState`, deliberately. The empty state below is the tutor's
         * only chance to explain that it withholds answers on purpose — its own comment says so —
         * and replacing that with "no model installed" would trade one missing fact for a worse
         * one. It also has to show when there *is* history: a conversation from a machine that has
         * since lost its model should still say why nothing answers.
         *
         * `!ready` covers both no-backend and backend-with-no-models. They need different fixes and
         * the status bar distinguishes them, but the sentence a user needs here is the same.
         */}
        {!runtime.loading && !runtime.ready && (
          <div className="flex items-start gap-3 rounded-xl border border-line-strong bg-void-2 px-4 py-3">
            <svg
              className="mt-0.5 h-4 w-4 flex-shrink-0 text-ink-2"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden
            >
            ...
            <div className="min-w-0">
              <p className="text-[12px] font-medium text-ink">{NO_MODEL_INSTALLED.title}</p>
              <p className="mt-0.5 text-[12px] leading-relaxed text-ink-2">
                {NO_MODEL_INSTALLED.body}
              </p>
              ...
              <a
                ...
                className="mt-1.5 inline-block text-[11px] text-ink underline underline-offset-2 transition-colors hover:text-ink-2"
              >
                ollama.com
              </a>
            </div>
      ...
      <div
        ...
        onDragOver={(e) => {
          if (imageFilesFrom(e.dataTransfer.items).length > 0) e.preventDefault();
        }}
        onDrop={(e) => {
          const files = imageFilesFrom(e.dataTransfer.items);
      ...
      >
          ...
          <ul className="mb-2 flex flex-wrap gap-2">
            {attachments.map((attachment) => (
              <li key={attachment.id} className="group relative">
                {/* eslint-disable-next-line @next/next/no-img-element -- a data: URL, not a file
                ...
                <button
                  ...
                  className="absolute -right-1.5 -top-1.5 h-4 w-4 rounded-full border border-line bg-ide-panel text-[10px] leading-[14px] text-ink-2 opacity-0 transition-opacity group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none"
                >
                  ×
                </button>
              </li>
        ...
        {attachError !== null && (
          // Above the composer, to the person choosing what to attach — not into the prompt.
          // The model does not need telling what it was not given.
          <p className="mb-2 text-[11px] text-diff-remove-ink">{attachError}</p>
        )}
        {visionNotice !== null && attachments.length > 0 && (
          <p className="mb-2 text-[11px] leading-relaxed text-ink-3">{visionNotice}</p>
        )}
        ...
        <div className="flex items-end gap-2 bg-ide-code rounded-2xl px-4 py-2.5 border border-line focus-within:border-line-strong transition-colors">
          <textarea
            ...
            onPaste={(e) => {
              // How a diagram actually arrives: a screenshot on the clipboard, with no file on
              // disk to drag.
          ...
          />
          {/*
            A picker as well as paste and drop.

            Paste is how most images arrive, and a feature reachable only by paste is a feature
            nobody finds — there is nothing on screen to suggest the tutor takes a picture at all.
          */}
          <label
            title="Attach a diagram or a loss curve"
            className={`flex-shrink-0 mb-0.5 cursor-pointer text-ink-3 transition-colors hover:text-ink ${
              attachments.length >= MAX_ATTACHMENTS ? "pointer-events-none opacity-30" : ""
            }`}
          >
            ...
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
              ...
              <circle cx="5.6" cy="6.2" r="1.1" stroke="currentColor" strokeWidth="1.3" />
              <path d="M2.4 11.4l3.3-3 2.2 2 2.5-2.6 3.2 3.4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </label>
          <button
            onClick={handleSend}
            className="flex-shrink-0 mb-0.5"
            disabled={isLoading || !canSend(input, attachments.length)}
          >
            <svg
              width="22"
              height="22"
              ...
              className={`transition-opacity ${
                isLoading || !canSend(input, attachments.length)
                  ? "opacity-30"
                  : "opacity-100"
              }`}
            >
