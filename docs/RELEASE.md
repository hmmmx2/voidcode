...
| | State | Why | Follow-up |
|---|---|---|---|
| **Code signing (Windows)** | Wired for **SignPath** (free for OSS) in `release.yml`'s `sign-windows` job, dormant until the owner's SignPath account is configured (the `SIGNPATH_*` variables). Until then installers ship unsigned and `Get-AuthenticodeSignature` reports `NotSigned`. | A certificate is an annual fee; SignPath is free for open source but needs one-time OSS onboarding. | Owner: finish SignPath onboarding, set the secret + vars. `docs/OPEN_QUESTIONS.md` Q-001. |
